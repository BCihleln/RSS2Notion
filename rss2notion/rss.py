...
def parse_rss(subscirption: Subscription) -> list[RSSEntry]:
    ...
    # 透過 fetcher 進行多階段獲取與解析
    parse_result = fetch_and_parse_feed(subscirption.url)

    # 如果有 bozo 错误但没有 entries，无法继续
    if parse_result.bozo:
        if parse_result.entries:
            log.info(f"   {subscirption.name} 解析異常，但成功提取 {len(parse_result.entries)} 条条目: {parse_result.bozo_exception}")
        else:
