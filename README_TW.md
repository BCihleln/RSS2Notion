...
- **Notion 訂閱管理** — 直接在 Notion 中新增、編輯、停用訂閱源，無需修改設定檔
- **Feed 內容渲染** — 將 RSS Feed 中已包含的 HTML 內容（`content` 或 `summary` 欄位）轉換為 Notion 塊，保留標題、清單、程式碼區塊、表格、引用及行內格式
...
- **錯誤追蹤** — 同步失敗時自動將帶有時間戳的錯誤 Callout 區塊附加到訂閱頁面；連續失敗次數達到閾值後升級為 `Error` 狀態
