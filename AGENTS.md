...
tests/
├── test_notion_validation.py  # Notion 屬性校驗與連結測試
└── test_rsshub.py             # RSSHub 連通性與 URL 自動簽名校驗測試
```
...
- **測試腳本規範**：所有測試與驗證腳本必須統一放置於 `tests/` 目錄下，不得散落在根目錄或 `scratch` 中。

## Commit 規範與工作流

### Commit 訊息格式
本項目採用語意化 Commit (Semantic Commits) 格式，格式如下：
`[type]: [description]`

常見的 `[type]` 類型：
- `feat`：新增功能 (Feature)
- `fix`：修復 Bug
- `docs`：文檔修改/補充 (Documentation)
- `refactor`：代碼重構 (Refactoring)
- `dev`：開發環境、輔助測試腳本或本地調試代碼的調整
