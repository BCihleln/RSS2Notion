...
- **Aggregated mode** — For high-frequency feeds, groups multiple articles as sub-items under a parent page using Notion's Sub-item feature; a single new article is written directly without a parent page
...
flowchart TD
    ...
    subgraph Phase2 [Phase 2 — Serial Write to Notion]
        ...
        M -- Yes --> M2{Article\ncount?}
        M2 -- 1 article --> N1
        M2 -- "≥2 articles" --> N3[Create Parent page\nWrite each as Sub-item]
        N1 --> O{Write result}
        N3 --> O
        O -- Success --> L
...
| `Aggregated` | checkbox | Read | Enable aggregated mode: uses Notion Sub-items to group articles under a parent page |
...
| `Aggregate Parent` | relation | Write | Sub-item parent page (aggregated mode; auto-generated by Notion Sub-items) |

> **Note:** `Aggregate Parent` and `Aggregated Articles` are Relation properties automatically created when you enable Sub-items on the database. The code only writes to `Aggregate Parent` (to set the parent page when creating sub-item articles).
...
- [lcoolcool/RSS2Notion](https://github.com/lcoolcool/RSS2Notion) — Fork Source
